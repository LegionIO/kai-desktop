...
1. **The pending-approval entry is deleted on any abort.** The ask_user gate
   (`electron/ipc/agent.ts:4219-4242`) awaits `registerPendingApproval(streamId, controller.signal)`.
   That controller is the _current turn's_. Anything that aborts it while the question is
