...
export const AdvancedSettings: FC<SettingsProps> = ({ config, updateConfig }) => {
  ...
  const shouldShowRecommendation =
    stats && stats.limitHitRate > 0.3 && stats.recommendation && stats.recommendation > advanced.maxSteps;
                ...
                <>
                  {stats.totalResponses === 0 ? (
                    <div className="flex items-start gap-2 text-xs text-muted-foreground">
                      <InfoIcon className="h-3.5 w-3.5 shrink-0 mt-0.5" />
                      <p>No usage data yet. Statistics will appear after you use Kai for a while.</p>
                    </div>
                  ) : (
                    <>
                      <div className="text-[10px] font-semibold text-foreground/90">Your Step Usage</div>
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1 space-y-2">
                              <p className="text-[10px] text-amber-900 dark:text-amber-100">
                                ...
                                {(stats.limitHitRate * 100).toFixed(0)}% of tasks. Consider increasing max steps to{' '}
                                <strong>{stats.recommendation}</strong> for better results.
                              </p>
                              <button
                                type="button"
                      ...
                      {!shouldShowRecommendation && stats.totalResponses > 0 && (
                        <div className="flex items-start gap-2 rounded-md bg-green-500/10 p-2 text-[10px] text-green-700 dark:text-green-300">
                          ...
                          <p>
                            Your current limit of <strong>{advanced.maxSteps} steps</strong> seems adequate. You rarely
                            hit the limit.
                          </p>
                        </div>
                      )}
