...
interface RuntimeOption {
  ...
  /** When false, the runtime runs tools unsupervised — surface an autonomy warning. */
...
const RUNTIME_OPTIONS: RuntimeOption[] = [
  ...
  },
  {
    ...
    icon: CpuIcon,
    perActionApproval: false, // pi runs tools unsupervised — no per-action hook
  },
  {
    id: 'mastra',
    label: 'Mastra',
      ...
      {RUNTIME_OPTIONS.map((option) => {
        ...
        return (
          <div key={option.id} className="grid gap-1">
            ...
            {isSelected && option.perActionApproval === false && <AutonomyWarning />}
          </div>
        );
      })}
    </div>
