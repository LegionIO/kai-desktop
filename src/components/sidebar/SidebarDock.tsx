import { useState, useRef, useCallback, type FC, type ReactNode, type MouseEvent as ReactMouseEvent } from 'react';
    ...
    <div className={cn('flex items-end border-t border-sidebar-border/80', className)}>
      ...
      >
        <div className="flex w-max items-end gap-0.5 px-2 mx-auto">
          {items.map((item) => (
            <DockIcon key={item.id} item={item} mouseX={mouseX} containerRef={scrollRef} />
          ))}
        </div>
      </div>
