 ...
 * Falls back to the shared Azure primary provider when no explicit embedding provider is configured.
 */
function buildEmbeddingProvider(config: AppConfig): EmbeddingResult | null {
  ...
  if (providerType === 'openai') {
    ...
    const modelId = embCfg?.model || 'text-embedding-3-small';
  ...
  if (embCfg?.azure?.endpoint && embCfg?.azure?.apiKey) {
    ...
    const modelId = embCfg.azure.deploymentName || embCfg?.model || 'text-embedding-3-small';
    return { embeddingModel: provider.embedding(modelId), modelId };
  }
