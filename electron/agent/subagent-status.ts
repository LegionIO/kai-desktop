 ...
 */
export async function readSubagentStatus(
  memory: Memory,
  threadId: string,
): Promise<SubagentStatusFields | null> {
...
export async function updateSubagentStatus(
  ...
  if (updates.status !== undefined && currentSub.status !== undefined) {
    ...
    if (!allowed.includes(updates.status)) {
      console.error(
        `[Subagent] Illegal status transition for ${threadId}: ${currentSub.status} -> ${updates.status}`,
      );
      return;
    }
  }
